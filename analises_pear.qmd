...
adapta_pe<-read.delim2("~/pear/DadosBiod.csv") # Dados do Adapta Brasil sobre Integridade dos Biomas
adapta_pe %>% 
...
left_join(sp_counts_mun, adapta_pe, by="code_muni")
...
```{r}
dados_comp_geom<-left_join(muni_pe,adapta_pe)
...
dados_comp_geom %>% 
  ...
  pca_sensib<-prcomp(dados_sensib[,-c(5,6)], scale = TRUE) 
  summary(pca_sensib)
  pca_sensib$rotation
...
cowplot::plot_grid(luc_map, luc_graph, ncol=1)
