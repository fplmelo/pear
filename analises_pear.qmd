...
### Indicadores individuais - Mudança de uso do solo

```{r}

dados_comp_geom %>% 
  select(starts_with("sl"), rds) %>% 
  pivot_longer(cols=starts_with("sl"), names_to="indic", values_to="value") %>% 
  ggplot()+geom_density(aes(value, colour = indic, fill=indic), alpha=0.5)+
  scale_fill_manual(values= col.map)+
